...
public:
  ...
  void rep_proxy_main(const char * front_ep, typename RM::Mode front_mode,
      const char * back_ep, typename RM::Mode back_mode) {
    proxy_main(ZMQ_ROUTER, front_ep, front_mode, ZMQ_DEALER, back_ep, back_mode);
  }

  void pull_proxy_main(const char * front_ep, typename RM::Mode front_mode,
      const char * back_ep, typename RM::Mode back_mode) {
    proxy_main(ZMQ_PULL, front_ep, front_mode, ZMQ_PUSH, back_ep, back_mode);
  }

  void pub_proxy_main(const char * front_ep, typename RM::Mode front_mode,
      const char * back_ep, typename RM::Mode back_mode) {
    proxy_main(ZMQ_XSUB, front_ep, front_mode, ZMQ_XPUB, back_ep, back_mode);
  }
